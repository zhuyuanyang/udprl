...
import pybulletgym
import time
def train(env_id, num_timesteps, seed, lr, entcoef, continue_train, nsteps, bins,cpus, tau):
    ...
    ppo2.learn(policy=policy, env=env, nsteps=nsteps, nminibatches=32,
        ...
        lr=lr,
        cliprange=0.2,
        total_timesteps=num_timesteps, tau= tau)
...
def main():
    ...
    logger.configure(dir = '../../exp/'+ str(args.env) + '/ppo_poisson' + '-'+ str(args.bins)+ '-'+ str(args.seed)+ '-'+ str(args.tau) + '-' +str(time_record))
    train(args.env, nsteps=args.nsteps, entcoef=args.entcoef, num_timesteps=args.num_timesteps, seed=args.seed, lr=args.lr, continue_train=args.continue_train, bins=args.bins, cpus = args.cpus, tau = args.tau)

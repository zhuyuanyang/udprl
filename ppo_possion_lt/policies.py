...
class MlpDiscretePolicy(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, reuse=False):  # pylint: disable=W0613
        ...
        with tf.compat.v1.variable_scope("model", reuse=reuse):
            ...
            tau = fc(h2, 'tau', 1, act=tf.nn.softplus, init_scale=0.01)

            tau = tf.clip_by_value(tau, 2,2.5)

            tau_tile = tf.tile(tf.expand_dims(tau, axis=-1), [1, actdim, bins])
            ...
            pdparam = gumbel_softmax(norm_softm_tiled_new,tau_tile)
        ...
        def poi(ob, *_args, **_kwargs):
            return sess.run(m,{X:ob})

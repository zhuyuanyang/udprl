...
class MlpDiscretePolicy(object):
    def __init__(self, sess, ob_space, ac_space, nbatch, nsteps, reuse=False): #pylint: disable=W0613
        ...
        X = tf.placeholder(tf.float32, ob_shape, name='Ob') #obs softplus
        with tf.variable_scope("model", reuse=reuse):
            h1 = fc(X, 'pi_fc1', nh=128, init_scale=np.sqrt(2), act=tf.nn.tanh)
            h2 = fc(h1, 'pi_fc2', nh=128, init_scale=np.sqrt(2), act=tf.nn.tanh)
            #pdparam = fc(h2, 'pi', pdtype.param_shape()[0], act=lambda x:x, init_scale=0.01)
            m = fc(h2, 'pi', bins*actdim, act=tf.nn.tanh, init_scale=0.01) # of size [batchsize, num-actions*bins]
            norm_softm = tf.nn.sigmoid(m) # of size [batchsize, num-actions*bins], initialized to be about uniform
            norm_softm_new = tf.reshape(norm_softm, [nbatch,-1,bins]) # of size [batchsize, num-actions, bins], initialized to be about uniform

            norm_softm_tiled = tf.tile(tf.expand_dims(norm_softm_new, axis=-1), [1,1,1,1])

...
class MultiCategoricalPd(Pd):
    def __init__(self, nvec, flat):
        self.flat = flat
        bins = 1 + nvec.max()
        acdim = [bins for x in range(0, nvec.shape[0], 1)]
        self.categoricals = list(map(CategoricalPd,
            tf.split(flat, acdim, axis=-1)))
    ...
    def neglogp(self, x):
        c = tf.add_n([p.neglogp(px) for p, px in zip(self.categoricals, tf.unstack(x, axis=-1))])
        return c
    def kl(self, other):
        return tf.add_n([p.kl(q) for p, q in zip(self.categoricals, other.categoricals)])
    def entropy(self):
        b = tf.add_n([p.entropy() for p in self.categoricals])
        return b
    def sample(self):
        a = tf.cast(tf.stack([p.sample() for p in self.categoricals], axis=-1), tf.int32)
        return a
    @classmethod
    def fromflat(cls, flat):
        raise NotImplementedError
